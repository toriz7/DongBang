...
router.get('/community/:page',function(req,res){
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      var dbo = db.db("mylab");
        ...
        if (err) throw err;
        if(req.session.user){
          res.render('community/:page', { email:req.session.user.email, name:req.session.user.name, board: result });
          db.close();
        }
        else{
          res.render('community/:page', { email:null, name:null, board: result });
          db.close();
        }
      });
    });
